...
Class MainWindow
    Dim m_CancellationTokenSource As System.Threading.CancellationTokenSource
    ...
    Private Async Sub Button_OpenPort_ClickAsync(sender As Object, e As RoutedEventArgs)
        ...
        Try
            ...
            If Await m_SpcInterface.InitializeAsync() Then
    ...
    Private Sub Button_GetLastHeartbeat_Click(sender As Object, e As RoutedEventArgs)
        Dim heartbeat As ReaderHeartbeat = m_SpcInterface.Heartbeat
    ...
    Private Sub Button_GetLastRawData_Click(sender As Object, e As RoutedEventArgs)
        Dim rawData As RawDataReceived = m_SpcInterface.DataReceived
        If rawData IsNot Nothing Then
            DecodeReceivedText(rawData.Data)
        End If
    End Sub

    Private Async Sub Button_GetHeartbeatAsync_ClickAsync(sender As Object, e As RoutedEventArgs)
        m_CancellationTokenSource.Cancel()
        AddLoggingText("Waiting asynchronously for Heartbeat...")
        Dim heartbeat As ReaderHeartbeat = Await m_SpcInterface.GetHeartbeatAsync(m_CancellationTokenSource.Token)
        If heartbeat IsNot Nothing Then
            AddLoggingText(String.Format("Heartbeat received: {0}, {1}", heartbeat.ReaderID, heartbeat.BatteryStatus))
            Dispatcher.Invoke(Sub()
                                  textBlock_ReaderID.Text = heartbeat.ReaderID.ToString()
    ...
    Private Async Sub Button_GetRawDataAsync_ClickAsync(sender As Object, e As RoutedEventArgs)
        m_CancellationTokenSource.Cancel()
        AddLoggingText("Waiting asynchronously for RawData...")
        Dim rawData As RawDataReceived = Await m_SpcInterface.GetDataReceivedAsync(m_CancellationTokenSource.Token)
        If rawData IsNot Nothing Then
            DecodeReceivedText(rawData.Data)
        End If
    End Sub
